...
- (void)strokeChart
{
    ...
    if (_showChartBorder) {
      ...
      _chartLevelLine.lineDashPattern = @[@6, @6];
      
      UIBezierPath *progressline1 = [UIBezierPath bezierPath];
      
      float unitHeight = (self.frame.size.height - _chartMarginTop - _chartMarginBottom - kXLabelHeight)/_maxY;
      [progressline1 moveToPoint:CGPointMake(_chartMarginLeft, unitHeight * (_maxY - _dottedLineValue) + _chartMarginTop)];
      [progressline1 addLineToPoint:CGPointMake(self.frame.size.width - _chartMarginRight, unitHeight * (_maxY - _dottedLineValue) + _chartMarginTop)];
      
      [progressline1 setLineWidth:1.0];
      [progressline1 setLineCapStyle:kCGLineCapSquare];
      _chartLevelLine.path = progressline1.CGPath;
      
      _chartLevelLine.strokeColor = [[UIColor grayColor] CGColor];
